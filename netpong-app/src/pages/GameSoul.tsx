    ...
    const OppCard = opponentUser ? (
        <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
                ...
                {isFriend === true && (
                    <span className="text-xs text-cyan-400 font-semibold mt-1">✓ Soul Bond</span>
                )}
            </div>
        </div>
    ) : null;
        ...
        <div className="relative h-[100svh] overflow-hidden">
            ...
            <div className="relative z-10 h-full flex flex-col items-center justify-center p-4 sm:p-6 gap-4 sm:gap-6 overflow-hidden">
                ...
                <div className="text-center space-y-1">
                    <h1
                        className="text-6xl md:text-7xl lg:text-8xl font-extrabold tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-gray-400 via-slate-300 to-cyan-400"
                        style={{ filter: 'drop-shadow(0 0 30px rgba(120,180,255,0.9))' }}
                    >
                        SOUL SOCIETY
                    </h1>
                    <p
                        className="text-slate-400 text-base md:text-lg font-bold tracking-[0.4em] uppercase"
                        style={{ textShadow: '0 0 40px rgba(200,230,255,1)' }}
                    >
                        THE SOUL SAVER
                    </p>
                </div>
                ...
                {error && (
                    <div className="bg-slate-950/80 border border-cyan-700 text-cyan-300 rounded-xl px-6 py-3 text-sm font-semibold">
                        ⚔️ {error}
                    </div>
                )}
                ...
                {screen === 'MODE_SELECT' && (
                    <div
                        className="bg-black/70 backdrop-blur-xl border border-slate-500/30 rounded-2xl p-8 flex flex-col items-center gap-6 w-full max-w-sm"
                        style={{ boxShadow: '0 0 30px rgba(120,180,255,0.1)' }}
                    >
                ...
                {screen === 'IN_QUEUE' && (
                    <div
                        className="bg-black/70 backdrop-blur-xl border border-slate-500/30 rounded-2xl p-10 flex flex-col items-center gap-6 w-full max-w-sm"
                        style={{ boxShadow: '0 0 30px rgba(120,180,255,0.1)' }}
                    >
                ...
                {screen === 'READY_CHECK' && (
                    <div
                        ...
                        style={{
                            ...
                            boxShadow: '0 0 30px rgba(120,180,255,0.2)',
                        }}
                    >
                        <div className="text-5xl animate-bounce">⚔️</div>
                        <p
                            className="font-extrabold text-2xl tracking-wide text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-slate-300 to-cyan-300"
                        ...
                        <p className="text-gray-400 text-sm">
                            You are on the{' '}
                            <span className={`font-bold ${side === 'left' ? 'text-cyan-400' : 'text-red-400'}`}>
                                {side.toUpperCase()}
                            </span>{' '}
                            side
                        </p>
                ...
                {screen === 'IN_GAME' && socketRef.current && (
                    <div className="relative w-full max-w-5xl flex flex-col gap-3">
                        {opponentUser && (
                            <div
                                className="w-full bg-black/70 backdrop-blur-xl border border-slate-500/20 rounded-2xl p-4 flex items-center justify-between gap-4"
                                style={{ boxShadow: '0 0 20px rgba(120,180,255,0.08)' }}
                            >
                                {side === 'left' ? MyCard : OppCard}
                                <div className="flex flex-col items-center flex-shrink-0">
                                    <span className="text-2xl">⚔️</span>
                                    <span className="text-[10px] font-bold text-slate-600 mt-1 tracking-widest uppercase">Soul Society</span>
                                </div>
                        ...
                        <div
                            className="aspect-video min-h-[250px] sm:min-h-[350px] md:min-h-[450px] rounded-2xl overflow-hidden border-2 border-slate-500/20"
                            style={{ boxShadow: '0 0 60px rgba(120,180,255,0.2)' }}
                        >
                            <SoulHockey
                                side={side}
                        ...
                        <button
                            onClick={exitGame}
                            className="w-full bg-slate-950/60 hover:bg-slate-900/70 border border-slate-700/50 text-slate-400 hover:text-slate-300 font-bold py-3 rounded-xl transition-all text-sm tracking-wide flex items-center justify-center gap-2"
                        >
                ...
                {screen === 'GAME_OVER' && gameOverData && (
                    <div
                        ...
                        style={{
                            border: `2px solid ${iWon ? 'rgba(0,229,255,0.4)' : 'rgba(255,23,68,0.4)'}`,
                            boxShadow: `0 0 40px ${iWon ? 'rgba(0,229,255,0.15)' : 'rgba(255,23,68,0.15)'}`,
                        }}
                    >
                        <div className="text-7xl">{iWon ? '⚔️' : '👻'}</div>
                        <h2
                            className="text-5xl font-extrabold tracking-wide text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-slate-300 to-yellow-300"
                            style={{ filter: `drop-shadow(0 0 20px ${iWon ? 'rgba(0,229,255,0.6)' : 'rgba(255,23,68,0.6)'})` }}
                        >
                            {iWon ? 'Bankai!' : 'Soul Lost...'}
                        </h2>
                ...
                {screen === 'GAME_ABORTED' && (
                    <div
                        ...
                        style={{
                            ...
                            boxShadow: '0 0 40px rgba(120,180,255,0.1)',
                        }}
                    >
                        <div className="text-7xl animate-bounce">👻</div>
                        <h2
                            className="text-3xl font-extrabold tracking-wide text-transparent bg-clip-text bg-gradient-to-r from-slate-300 to-cyan-400"
                        ...
                        <div className="w-full bg-slate-900/50 border border-slate-700/30 rounded-xl px-4 py-3 text-center">
                            <p className="text-gray-500 text-sm">{abortData?.reason || 'A soul abandoned the battlefield.'}</p>
                        </div>
                        ...
                        >
                            <p className="font-bold text-sm text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-yellow-300">
                                ⚔️ Your soul reigns supreme!
                            </p>
                        </div>
