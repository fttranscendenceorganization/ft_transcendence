...
export default function GameKitty() {
    ...
    const MyCard = (
        <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
                <span className="text-pink-400 font-bold text-lg tracking-wide">{myName}</span>
                {playerStats && (
                    <div className="mt-1 grid grid-cols-2 gap-x-3 text-xs text-pink-300/70">
                        <span>Games <span className="text-white font-semibold">{playerStats.total}</span></span>
                        <span>Wins <span className="text-pink-400 font-semibold">{playerStats.wins}</span></span>
                        <span>Losses <span className="text-rose-400 font-semibold">{playerStats.losses}</span></span>
    ...
    const OppCard = opponentUser ? (
        <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
                <span className="text-rose-400 font-bold text-lg tracking-wide">{opponentUser.username}</span>
                {opponentStats ? (
                    <div className="mt-1 grid grid-cols-2 gap-x-3 text-xs text-rose-300/70">
                        <span>Games <span className="text-white font-semibold">{opponentStats.total}</span></span>
                        <span>Wins <span className="text-pink-400 font-semibold">{opponentStats.wins}</span></span>
        ...
        return (
            <div className="relative h-[100svh] overflow-hidden">
                ...
                <div className="relative z-10 h-full flex flex-col items-center justify-center p-6 gap-6">
                    ...
                    <h2 className="text-3xl font-extrabold text-pink-400" style={{ textShadow: '0 0 20px rgba(255,105,180,0.6)' }}>Game Already Active</h2>
                    ...
                    <button onClick={() => navigate('/kitty-cat')} className="mt-4 bg-pink-600 hover:bg-pink-500 text-white font-bold py-3 px-8 rounded-xl transition-all hover:scale-105" style={{ boxShadow: '0 0 20px rgba(255,105,180,0.3)' }}>← Go Back</button>
                </div>
            </div>
        );
    }
